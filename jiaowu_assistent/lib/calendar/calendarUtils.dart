...
///公历节日
const List<String> _solarCalendar = [
  "0101元旦",
  "0214情人节",
  "0308妇女节",
  "0312植树节",
...
/// 传统农历节日
const List<String> _traditionFestivalStr = [
  "除夕",
  "0101春节",
  "0115元宵",
  "0505端午",
...
/// 特殊节日的数组
const List<String> _specialFestivalStr = [
  "母亲节",
  "父亲节",
  "感恩节",
];
...
//    }
  ...
  static List<int> solarToLunar(int year, int month, int day) {
    ...
    int index = year - _solar[0];
    int data = (year << 9) | (month << 5) | (day);
    int solar11;
    if (_solar[index] > data) {
      index--;
    }
    solar11 = _solar[index];
    int y = getBitInt(solar11, 12, 9);
    int m = getBitInt(solar11, 4, 5);
    int d = getBitInt(solar11, 5, 0);
    int offset = solarToInt(year, month, day) - solarToInt(y, m, d);

    int days = _lunarMonthDays[index];
    int leap = getBitInt(days, 4, 13);

    int lunarY = index + _solar[0];
    int lunarM = 1;
    int lunarD;
    offset += 1;
...
//  }
  ...
  static String gregorianFestival(int month, int day) {
    ...
    String solar = "";
    for (String aMSolarCalendar in _solarCalendar) {
  ...
  static String getTraditionFestival(int year, int month, int day) {
    int daysInLunarMonth(int year, int month) {
      if ((lunarInfo[year - 1900] & (0x100000 >> month)) == 0)
        return 29;
      else
        return 30;
    }
    ...
    if (month == 12) {
      ...
      if (day == count) {
        return _traditionFestivalStr[0]; //除夕
      }
    }
    String text = getString(month, day);
    String festivalStr = "";
    for (String festival in _traditionFestivalStr) {
  ...
  static String getSpecialFestival(int year, int month, int day) {
    List<String> getSpecialFestivals(int year) {
      ...
      if (startDiff == 7) {
        festivals[0] =
            _dateToString(year, 5, startDiff + 1) + _specialFestivalStr[0];
      } else {
        festivals[0] =
            _dateToString(year, 5, startDiff + 7 + 1) + _specialFestivalStr[0];
      }
      ...
      if (startDiff == 7) {
        festivals[1] =
            _dateToString(year, 6, startDiff + 7 + 1) + _specialFestivalStr[1];
      } else {
        festivals[1] = _dateToString(year, 6, startDiff + 7 + 7 + 1) +
            _specialFestivalStr[1];
      }
      ...
      startDiff = 7 - week + 1;
      if (startDiff <= 2) {
        festivals[2] = _dateToString(year, 11, startDiff + 21 + 5) +
            _specialFestivalStr[2];
      } else {
        festivals[2] = _dateToString(year, 11, startDiff + 14 + 5) +
            _specialFestivalStr[2];
      }
      return festivals;
    }
